...
# PCIe Gen 4 SerDes 行為級 vs 晶體管級全方位嚴謹對比驗證腳本
files = {
    "run_full_python_vs_hspice_co_verification.py": '''"""
...
def plot_full_co_verification(t_vec, v_tx_py, v_tx_sp, v_ctle_py, v_ctle_sp, dfe_v_py, dfe_v_sp, filename="python_vs_hspice_full_comparison.png"):
    ...
    axes.plot(t_ns, v_ctle_py[:500], 'b-', linewidth=1.5, label="Python CTLE Output")
    axes.plot(t_ns, v_ctle_sp[:500], 'g--', linewidth=1.5, label="HSPICE CTLE Circuit (0.18um)")
    axes.set_title("2. RX CTLE Output Waveform Comparison", fontsize=11, fontweight='bold')
    axes.set_xlabel("Time (ns)", fontsize=10)
    axes.set_ylabel("Differential Voltage (V)", fontsize=10)
    ...
    for i in range(15, min(num_eyes_py - 15, 150)):
        ...
        axes.plot(eye_time, seg, color='blue', alpha=0.15, linewidth=0.8)
    axes.set_title("3. Python Behavioral Model Restored Eye Diagram", fontsize=11, fontweight='bold')
    axes.set_xlabel("Time (ps)", fontsize=10)
    axes.set_ylabel("Differential Voltage (V)", fontsize=10)
    axes.grid(True, linestyle='--', alpha=0.5)
    ...
    for i in range(15, min(num_eyes_sp - 15, 150)):
        ...
        axes.plot(eye_time, seg, color='darkgreen', alpha=0.15, linewidth=0.8)
    axes.set_title("4. HSPICE Transistor-Level Circuit Restored Eye Diagram", fontsize=11, fontweight='bold')
    axes.set_xlabel("Time (ps)", fontsize=10)
    axes.set_ylabel("Differential Voltage (V)", fontsize=10)
    axes.grid(True, linestyle='--', alpha=0.5)
    axes.set_ylim(-0.8, 0.8)
...
def main():
    ...
    plot_full_co_verification(t_vec, v_tx_py, v_tx_sp, v_vga_py, v_vga_sp, dfe_v_py, dfe_v_sp, "python_vs_hspice_full_comparison.png")
    ...
    print(f"  DFE Tap 2 Weight (h2)        | {rx_py.dfe.taps:17.4f} | {rx_sp.dfe.taps:17.4f} | Converged (LMS)")
...
print("[+] setup_full_co_verification.py 建立完成！")