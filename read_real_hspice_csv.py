...
def plot_real_hspice_eye_diagram(hspice_time, hspice_v_ctle, filename="real_hspice_restored_eye.png"):
    ...
    # 1. 自動濾除前 1ns 的直流過渡期
    if len(hspice_time) != len(hspice_v_ctle):
        n = min(len(hspice_time), len(hspice_v_ctle))
        hspice_time = hspice_time[:n]
        hspice_v_ctle = hspice_v_ctle[:n]
...
def main():
    ...
    try:
        ...
        if data.shape[1] < 2:
            raise ValueError("CSV 至少需要兩欄：時間與電壓")

        data = np.asarray(data, dtype=float)
        hspice_time = data[:, 0].astype(float).reshape(-1)
        hspice_v_ctle = data[:, 1].astype(float).reshape(-1)

        valid_mask = np.isfinite(hspice_time) & np.isfinite(hspice_v_ctle)
        hspice_time = hspice_time[valid_mask]
        hspice_v_ctle = hspice_v_ctle[valid_mask]

        if len(hspice_time) < 2:
            raise ValueError("有效的 HSPICE 時間/電壓資料點不足")

        order = np.argsort(hspice_time)
        hspice_time = hspice_time[order]
        hspice_v_ctle = hspice_v_ctle[order]
    ...
    rx_hspice = RXTop(ctle_boost_db=0.0, dfe_taps=5, enable_non_idealities=True)
    ...
    print(f"  DFE Tap 2 權重 (h2)          | {rx_hspice.dfe.taps[1]:18.4f}")
    print(f"  實測全鏈路 BER               | {ber_sp:18.6e}")
    print("HSPICE bits expected:", len(np.load("tx_bits.npy")))
    print("CSV duration bits:", csv_duration_sec / PCIeGen4Config.UI)
    print("="*75)
